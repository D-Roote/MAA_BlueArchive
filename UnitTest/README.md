...
UI 테스트는 시작 화면의 구분선·외곽 여백, 밝은/어두운 테마의 경로 입력칸·버튼 정렬, 창 크기별 가로 스크롤 발생 여부를 확인합니다. 설정 탐색은 부분적으로 보이는 최상단 카드, 스크롤 끝, 스크롤이 없는 화면, 직접 클릭한 항목의 선택 유지를 구분해 검증합니다. 자동 실행 작업의 제거·복원과 실행별 로그 초기화 설정의 기본값·저장·적용도 함께 검증합니다.
